...
def save_to_cache(url: str, content: str, metadata: dict):
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la sauvegarde dans le cache: {str(e)}", exc_info=True
        )
...
def load_from_cache(url: str) -> tuple:
    ...
    if os.path.exists(cache_path):
        try:
            ...
            logger.error(
                f"Erreur lors du chargement depuis le cache: {str(e)}", exc_info=True
            )
    else:
...
def get_pokepedia_urls(max_pages: int = MAX_PAGES) -> List[str]:
    ...
    while len(all_urls) < max_pages:
        request_count += 1
        logger.debug(
            f"Requête API #{request_count} - URLs trouvées: {len(all_urls)}/{max_pages}"
        )

        if continue_token:
            params["apcontinue"] = continue_token
            logger.debug(
                f"Utilisation du token de continuation: {continue_token[:20]}..."
            )
        ...
        try:
            ...
            if "continue" in data and len(all_urls) < max_pages:
                ...
                logger.debug(
                    f"Pause de {REQUEST_DELAY} secondes avant la prochaine requête"
                )
            ...
            logger.error(
                f"Erreur lors de la récupération des URLs: {str(e)}", exc_info=True
            )
            break
...
def clean_html_content(html_content: str) -> str:
    ...
    for element in soup.find_all(
        ["script", "style", "table", "div", "nav", "footer", "header"]
    ):
        element.decompose()
        elements_removed += 1
...
def retrieve_pokepedia_documents(max_pages: int = MAX_PAGES) -> List[Document]:
    ...
    for i in range(0, len(urls), BATCH_SIZE):
        ...
        logger.info(
            f"Traitement du lot {batch_num}/{total_batches} ({len(batch_urls)} URLs)"
        )
        ...
        try:
            ...
            for url in batch_urls:
                ...
                if cached_content:
                    ...
                    try:
                        ...
                        if docs:
                            doc = docs[0]
                            doc.metadata.update({"source": "pokepedia", "url": url})
                        ...
                        logger.error(
                            f"Erreur lors du chargement de {url}: {str(e)}",
                            exc_info=True,
                        )

            all_documents.extend(batch_documents)
            logger.info(
                f"Lot {batch_num} terminé. Documents dans ce lot: {len(batch_documents)}"
            )
            ...
            logger.error(
                f"Erreur lors du traitement du lot {batch_num}: {str(e)}", exc_info=True
            )
            continue

    logger.info(
        f"Récupération terminée. Nombre total de documents: {len(all_documents)}"
    )
